...
const getCleanModuleName = (exam) => {
    // 1. Priority: Arabic Name
    if (exam.lib_elp_arb) return exam.lib_elp_arb;

    // 2. Fallback: Clean French Name (Deduplicate)
...
const StatCard = ({ title, value, icon, color, loading, subtitle, onClick }) => (
  ...
  >
    ...
    <CardContent sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, '&:last-child': { pb: 2 } }}>
      ...
      <Box sx={{ p: 1.5, borderRadius: 3, bgcolor: (theme) => theme.palette[color].light, color: (theme) => theme.palette[color].contrastText || theme.palette[color].dark, opacity: 0.8, display: 'flex', zIndex: 1 }}>
        {icon}
      </Box>
    </CardContent>
  </Card>
    ...
    <>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.5 }}>
             <Typography 
                variant="subtitle1" 
                sx={{ 
                    fontWeight: 'bold', lineHeight: 1.3, color: '#000', fontSize: '11pt', flex: 1, wordBreak: 'break-word',
                        ...
                        {Object.entries(timeSlots).map(([startTime, examsInSlot]) => {
                            ...
                            return (
                                <TableRow key={startTime} sx={{ verticalAlign: 'top' }}>
                                    <TableCell sx={{ width: '110px', bgcolor: '#f8f9fa', verticalAlign: 'middle', textAlign: 'center', borderRight: '1px solid #e0e0e0', '@media print': { width: '90px', bgcolor: '#eee !important' } }}>
                                            ...
                                            <Grid container spacing={2} sx={{ '@media print': { display: 'none' } }}>
                                                {mergedExams.map((exam, i) => (
                                                    <Grid item xs={12} md={6} lg={4} xl={3} key={i}>
                                                        <Card variant="outlined" sx={{ bgcolor: getExamColor(exam.module_code), borderColor: 'rgba(0,0,0,0.12)', cursor: 'pointer', height: 'auto', minHeight: '100%', transition: 'transform 0.1s', '&:hover': { transform: 'translateY(-2px)', boxShadow: 3 } }} onClick={() => onSelectExam(exam)}>
...
const ExamPlanning = () => {
  ...
  const handleAutoDistribute = () => {
    ...
    const ratio = Math.min(allGroupStudents.length / totalCapacity, 1);
    const newSessions = [];
    let assignedSoFar = 0;
    ...
    let remainder = allGroupStudents.length - assignedSoFar;
    if (remainder > 0) {
  ...
  // --- HANDLE EXAM CLICK (SEPARATE LISTS PER ROOM) ---
  const handleExamClick = async (exam) => {
    setCurrentExam(exam);
    setSurveillantName(exam.professor_name || ''); 
    ...
    setStudentDialog({ 
        ...
        byLocation: [], 
        loading: true, 
        title: `${exam.module_name} (${locString})` 
    });
    
    try {
        const ids = exam.ids || [exam.id];
        const promises = ids.map(id => adminAPI.getExamStudents(id));
        const results = await Promise.all(promises);
        
        // 1. All Students (for Display)
        const allStudentsMap = new Map();
        results.flat().forEach(s => {
            if (!allStudentsMap.has(s.cod_etu)) allStudentsMap.set(s.cod_etu, s);
        });
        const sortedAll = Array.from(allStudentsMap.values()).sort((a, b) => 
            a.lib_nom_pat_ind.localeCompare(b.lib_nom_pat_ind)
        );

        // 2. Per-Location Data (for PDF)
        const byLocationData = [];
        ids.forEach((id, index) => {
            const locName = exam.all_locations ? exam.all_locations[index] : exam.location;
            const studentsForLoc = results[index].sort((a, b) => a.lib_nom_pat_ind.localeCompare(b.lib_nom_pat_ind));
            
            byLocationData.push({
                location: locName,
                students: studentsForLoc
            });
        });
        ...
        setStudentDialog(p => ({ ...p, loading: false }));
    }
  };
 ...
 const handleSaveSurveillant = async () => {
  ...
  // --- PDF GENERATION WITH TEMPLATE & ARABIC FONT ---
  const handleGeneratePDF = async () => {
    ...
    try {
        ...
        doc.setFont('Amiri'); // Set global font
        ...
        setError("Erreur lors de la génération du PDF (Police 'Amiri' manquante ?)");
    } finally {
        setLoading(false);
    }
  };
    ...
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }} className="no-print">
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            ...
            <Button startIcon={<SyncIcon />} variant="outlined" color="secondary" onClick={handleRefreshParticipants}>
                Sync Listes
            </Button>

            <Button startIcon={<DownloadIcon />} variant="contained" color="success" onClick={handleExport} disabled={loading}>
      ...
      <Grid container spacing={2} sx={{ mb: 4 }} className="no-print">
        ...
        <Grid item xs={12} sm={6} md={2.4}>
           <StatCard title="Conflits Horaire" value={conflictCount} subtitle="Étudiants en doublon" icon={<WarningIcon />} color={conflictCount > 0 ? "error" : "success"} loading={loading} onClick={handleShowConflicts} />
        </Grid>
      </Grid>
        ...
        <Dialog open={studentDialog.open} onClose={() => setStudentDialog(prev => ({ ...prev, open: false }))} maxWidth="md" fullWidth className="no-print">
          ...
          <DialogContent dividers>
             ...
             <Box sx={{ mb: 3, p: 2, bgcolor: '#f5f5f5', borderRadius: 1, display: 'flex', gap: 2, alignItems: 'center' }}>
                <Autocomplete
                  freeSolo
                  options={AVAILABLE_PROFESSORS}
                  value={surveillantName}
                  onInputChange={(e, newVal) => setSurveillantName(newVal)}
                  renderInput={(params) => <TextField {...params} label="Professeur / Surveillant" size="small" />}
                  sx={{ flex: 1 }}
                />
                <Button variant="contained" color="primary" startIcon={<SaveIcon />} onClick={handleSaveSurveillant}>
                  Enregistrer
                </Button>
             </Box>
          ...
          <DialogActions>
            <Button variant="outlined" color="error" startIcon={<PdfIcon />} onClick={handleGeneratePDF}>
                Générer PDF
            </Button>
